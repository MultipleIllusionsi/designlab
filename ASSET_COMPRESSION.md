...
- **Видео — главное.** На детальной странице видео отдаётся ImageKit «как есть» (`orig-true`, без трансформации), т.е. **вес исходника = вес трафика.** Поэтому видео и сжимаем агрессивно: даунскейл до **1080p** (длинная сторона ≤ 1920px — стандарт для веба, чётко в лайтбоксе) + перекодирование H.264 с разумным качеством. Звук вырезаем — ролики немые.
...
Заливаешь в ImageKit файлы из **`compressed/`** (не из `original/`), с теми же именами. `original/` держи локально как бэкап. Папка `design_lab_assets/` в `.gitignore` — в репозиторий эти файлы не попадают.
